...
- WSL2 tmux processes: Go `ks-apiserver` on `19090`, Java `ghcloud/platform.jar` including UUC on `9000`, frontend `ghbf-gstack` dev server on `8000`.
...
- Treat `ghscheduler-mysql` as the canonical local MySQL. It contains `ghks`, `ghlmc`, `ghscheduler`, and `ghdsp`.
...
- For Pod-to-local development integration, expose Windows `0.0.0.0` portproxy entries to the current WSL IP. The common ports are:
  - `8000` frontend
  - `8080`, `8091`, `8100`, `3100` auxiliary local development services
...
- WSL listeners on `8000`, `9000`, `19090`
